    ...
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Manage your event offerings</p>
        </div>
      </div>
            ...
            {events.map((event: any) => (
              <tr key={event.id} className="hover:bg-gray-50">
                <td className="px-6 py-4">
                  ...
                  <div className="text-sm text-gray-500">{event.tagline}</div>
                </td>
                <td className="px-6 py-4 text-sm text-gray-600">{event.packages?.length || 0}</td>
                <td className="px-6 py-4">
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                    event.featured ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                  }`}>
